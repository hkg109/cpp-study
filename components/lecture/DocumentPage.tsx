...
export async function DocumentPage({ document }: { document: ContentDocument }) {
  ...
  return <div className="document-grid"><article className="document-main">
    <nav className="breadcrumb" aria-label="현재 위치"><Link href={`/${document.kind}`}>{label}</Link><ChevronRight size={13} /><span>Week {String(document.week).padStart(2, "0")}</span></nav>
