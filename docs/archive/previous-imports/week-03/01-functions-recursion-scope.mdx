---
title: "함수, 재귀, 스코프"
description: 함수와 재귀, 변수의 스코프와 저장 기간, 표준 라이브러리와 네임스페이스를 학습합니다.
week: 3
order: 1
published: true
---

> 함수는 큰 문제를 작은 문제로 나누고, 반복되는 코드를 재사용하기 위한 프로그램의 구성 단위이다.

## 핵심 요약

- 함수는 **입력값을 받아 작업을 수행하고 결과를 반환**할 수 있다.
- 함수 원형을 사용하면 함수의 실제 정의보다 앞에서 해당 함수를 호출할 수 있다.
- 변수는 선언된 위치에 따라 접근 가능한 범위와 메모리에 존재하는 기간이 달라진다.
